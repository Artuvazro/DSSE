...
namespace Devil_Survivor_Script_Editor
{
    class tagParser
    {
        public static void tagParse (TextBox Box1, TextBlock Box2)
        {
            ...
            if (((Box1.Text.Contains("[color1]")) || (Box1.Text.Contains("[color9]")) || (Box1.Text.Contains("[color2]")) || (Box1.Text.Contains("[color3]"))  || (Box1.Text.Contains("[color5]"))) && (Box1.Text.Contains("[/color]")))
            {
                ...
                for (int i = 0; i < tagsCount; i++)
                {
                    ...
                    else if (tagsFound1[i].Groups[0].Value == "[color1]") myYellowRun[i].Foreground = new SolidColorBrush(Color.FromRgb(168, 170, 27));
                    ...
                    if (beforeText.ElementAtOrDefault(i) != null) Box2.Inlines.Add(beforeText[i].Replace("[color1]", "").Replace("[color9]", "").Replace("[/color]", "").Replace("[color2]", "").Replace("[color3]","").Replace("[color5]","").Replace("[new]", ""));
                    Box2.Inlines.Add(myYellowRun[i]);
                    Box2.Inlines.Add(afterText[i].Replace("[color1]", "").Replace("[color9]", "").Replace("[/color]", "").Replace("[color2]", "").Replace("[color3]","").Replace("[color5]","").Replace("[new]",""));
                }
